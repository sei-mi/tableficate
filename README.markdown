...
3. The `_table.html.erb` theme partial has been renamed to `_table_for.html.erb`. Additionally, the `_table.html.erb` partial name is now used for the new `tableficate_table_tag` helper. Running `rails generate tableficate:theme NAME` for an existing theme will provide an interactive means of updating your existing theme.
4. The `_table_for.html.erb` theme partial uses the `tableficate_filter_form_tag` helper. A new partial, `filters/_form.html.erb` was created for the new helper. It can be easily added by running `rails generate tableficate:theme NAME` on an existing theme.
