...
describe Tableficate::InputFilter do
  ...
  it 'should use a template based on the type if one exists' do
    Tableficate::InputFilter.new(@table, :first_name, label: 'First', field_options: {type: 'email'}).template.should == 'input_filter'

    file = File.open('app/views/tableficate/_email_input_filter.html.erb', 'w')
    Tableficate::InputFilter.new(@table, :first_name, label: 'First', field_options: {type: 'email'}).template.should == 'email_input_filter'
